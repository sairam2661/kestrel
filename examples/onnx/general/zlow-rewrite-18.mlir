...
#map4 = affine_map<() -> (5)>
"builtin.module"() ({
  "func.func"() <{function_type = (memref<5x10xf16, #map>) -> memref<10x5xf16, #map>, sym_name = "should_not_rewrite_unstick_transpose_stick_3"}> ({
  ^bb0(%arg0: memref<5x10xf16, #map>):
    ...
    ^bb0(%arg1: index):
      "affine.for"() <{lowerBoundMap = #map2, operandSegmentSizes = array<i32: 0, 0, 0>, step = 1 : index, upperBoundMap = #map3}> ({
      ^bb0(%arg2: index):
        %3 = "affine.load"(%1, %arg2, %arg1) <{map = #map1}> : (memref<10x5xf32>, index, index) -> f32
        "affine.store"(%3, %1, %arg2, %arg1) <{map = #map1}> : (f32, memref<10x5xf32>, index, index) -> ()
        "affine.yield"() : () -> ()
      }) : () -> ()
      "affine.yield"() : () -> ()
