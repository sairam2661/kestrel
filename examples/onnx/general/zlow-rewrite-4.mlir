...
"builtin.module"() ({
  "func.func"() <{function_type = (memref<?x?xf16, #map>) -> memref<?x?xf16, #map>, sym_name = "unstick_stick_removal"}> ({
  ^bb0(%arg0: memref<?x?xf16, #map>):
    ...
    %7 = "memref.alloc"(%6, %5) <{alignment = 4096 : i64, operandSegmentSizes = array<i32: 2, 0>}> : (index, index) -> memref<?x?xf16, #map>
    "zlow.stick"(%4, %7) {layout = "2D"} : (memref<?x?xf32>, memref<?x?xf16, #map>) -> ()
    "func.return"(%7) : (memref<?x?xf16, #map>) -> ()
  }) : () -> ()
}) : () -> ()
