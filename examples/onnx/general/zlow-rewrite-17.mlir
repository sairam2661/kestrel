...
#map4 = affine_map<() -> (5)>
"builtin.module"() ({
  "func.func"() <{function_type = (memref<5x10xf16, #map>) -> memref<10x5xf16, #map>, sym_name = "should_not_rewrite_unstick_transpose_stick_2"}> ({
  ^bb0(%arg0: memref<5x10xf16, #map>):
    ...
    ^bb0(%arg1: index):
      ...
      ^bb0(%arg2: index):
        %3 = "affine.load"(%0, %arg1, %arg2) <{map = #map1}> : (memref<5x10xf32>, index, index) -> f32
        "affine.store"(%3, %0, %arg1, %arg2) <{map = #map1}> : (f32, memref<5x10xf32>, index, index) -> ()
        "affine.yield"() : () -> ()
      }) : () -> ()
      "affine.yield"() : () -> ()
    }) : () -> ()
