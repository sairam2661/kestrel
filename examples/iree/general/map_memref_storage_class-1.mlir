#executable_target_opencl_spirv_fb = #hal.executable.target<"opencl-spirv", "opencl-spirv-fb", {iree_codegen.target_info = #iree_gpu.target<arch = "", features = "spirv:v1.3,cap:Kernel", wgp = <compute =  fp32|int32, storage =  b32, subgroup =  shuffle|arithmetic, subgroup_size_choices = [64], max_workgroup_sizes = [128, 128, 64], max_thread_count_per_workgroup = 128, max_workgroup_memory_bytes = 16384, max_workgroup_counts = [65535, 65535, 65535]>>}>
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "opencl_client_api"}> ({
    ...
    "func.return"() : () -> ()
  }) {hal.executable.target = #executable_target_opencl_spirv_fb} : () -> ()
}) : () -> ()
