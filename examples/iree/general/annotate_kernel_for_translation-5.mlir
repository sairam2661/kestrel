...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "test_kern_arg", sym_visibility = "public"}> ({
    "hal.executable.variant"() <{sym_name = "rocm_hsaco_fb", sym_visibility = "public", target = #executable_target_rocm_hsaco_fb}> ({
      "hal.executable.export"() <{layout = #pipeline_layout, ordinal = 0 : index, subgroup_size = 64 : index, sym_name = "test_kern_arg", sym_visibility = "public", workgroup_size = [128 : index, 2 : index, 1 : index]}> ({
      ^bb0(%arg1: !hal.device):
      ...
      "builtin.module"() ({
        "llvm.func"() <{CConv = #llvm.cconv<ccc>, function_type = !llvm.func<void (i32)>, linkage = #llvm.linkage<external>, sym_name = "test_kern_arg", unnamed_addr = 0 : i64, visibility_ = 0 : i64}> ({
        ^bb0(%arg0: i32):
          "llvm.return"() : () -> ()
        }) {iree_codegen.denormal_fp_math_f32 = #iree_codegen.denormal_fp_math<"preserve-sign">, llvm_func_attrs = {check_attr}} : () -> ()
      }) : () -> ()
