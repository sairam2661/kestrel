    ...
    "hal.executable.variant"() <{sym_name = "vulkan", target = #executable_target_vulkan_spirv_fb}> ({
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = () -> (), sym_name = "interface_wg_size"}> ({
          ...
          %3 = "hal.interface.binding.subspan"() <{binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 0>}> : () -> memref<64x64xf32, #spirv.storage_class<StorageBuffer>>
          "memref.store"(%0, %3, %1, %2) : (f32, memref<64x64xf32, #spirv.storage_class<StorageBuffer>>, index, index) -> ()
          "func.return"() : () -> ()
        }) : () -> ()
