...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "expanded_matmul_transpose_b_executable"}> ({
    "hal.executable.variant"() <{sym_name = "rocm", target = #executable_target_rocm_hsaco_fb}> ({
      "hal.executable.export"() <{layout = #pipeline_layout, sym_name = "expanded_matmul_transpose_b", sym_visibility = "public"}> ({
      ^bb0(%arg5: !hal.device):
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = () -> (), sym_name = "expanded_matmul_transpose_b"}> ({
          ...
          ^bb0(%arg0: f16, %arg1: f16, %arg2: f16):
            ...
            "linalg.yield"(%11) : (f16) -> ()
          }) {lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_F32_16x16x16_F16>, promote_operands = [0, 1], reduction = [0, 0, 0, 0, 128], subgroup_basis = [[1, 1, 1, 4, 1], [0, 1, 2, 3, 4]], workgroup = [1, 1, 64, 64, 0]}>} : (tensor<2x64x2048xf16>, tensor<10x64x2048xf16>, tensor<2x10x64x64xf16>) -> tensor<2x10x64x64xf16>
