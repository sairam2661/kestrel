...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "matmul_391x384x384_f32"}> ({
    ...
    ^bb0(%arg0: index):
      ...
      ^bb0(%arg1: index):
        ...
        ^bb0(%arg2: index, %arg3: vector<8x32xf32>):
          %34 = "vector.transfer_read"(%24, %1, %arg2, %0) <{in_bounds = [false, false], operandSegmentSizes = array<i32: 1, 2, 1, 0>, permutation_map = #map3}> : (memref<?x384xf32, strided<[384, 1], offset: ?>>, index, index, f32) -> vector<8x16xf32>
          %35 = "vector.transfer_read"(%22, %arg2, %arg1, %0) <{in_bounds = [true, true], operandSegmentSizes = array<i32: 1, 2, 1, 0>, permutation_map = #map3}> : (memref<384x128xf32, strided<[384, 1], offset: ?>>, index, index, f32) -> vector<16x32xf32>
          %36 = "vector.contract"(%34, %35, %arg3) <{indexing_maps = [#map4, #map5, #map6], iterator_types = [#vector.iterator_type<parallel>, #vector.iterator_type<parallel>, #vector.iterator_type<reduction>], kind = #vector.kind<add>}> : (vector<8x16xf32>, vector<16x32xf32>, vector<8x32xf32>) -> vector<8x32xf32>
          "scf.yield"(%36) : (vector<8x32xf32>) -> ()
        }) : (index, index, index, vector<8x32xf32>) -> vector<8x32xf32>
        %28 = "vector.transfer_read"(%21, %arg1, %0) <{in_bounds = [true], operandSegmentSizes = array<i32: 1, 1, 1, 0>, permutation_map = #map7}> : (memref<128xf32, strided<[1], offset: ?>>, index, f32) -> vector<32xf32>
        %29 = "vector.broadcast"(%28) : (vector<32xf32>) -> vector<8x32xf32>
        %30 = "arith.addf"(%29, %27) <{fastmath = #arith.fastmath<none>}> : (vector<8x32xf32>, vector<8x32xf32>) -> vector<8x32xf32>
        %31 = "arith.minimumf"(%30, %8) <{fastmath = #arith.fastmath<none>}> : (vector<8x32xf32>, vector<8x32xf32>) -> vector<8x32xf32>
        %32 = "arith.maximumf"(%31, %7) <{fastmath = #arith.fastmath<none>}> : (vector<8x32xf32>, vector<8x32xf32>) -> vector<8x32xf32>
        %33 = "memref.subview"(%20, %arg0, %arg1, %23) <{operandSegmentSizes = array<i32: 1, 2, 1, 0>, static_offsets = array<i64: -9223372036854775808, -9223372036854775808>, static_sizes = array<i64: -9223372036854775808, 32>, static_strides = array<i64: 1, 1>}> : (memref<?x128xf32, strided<[384, 1], offset: ?>>, index, index, index) -> memref<?x32xf32, strided<[384, 1], offset: ?>>
        "vector.transfer_write"(%32, %33, %1, %1) <{in_bounds = [false, true], operandSegmentSizes = array<i32: 1, 1, 2, 0>, permutation_map = #map3}> : (vector<8x32xf32>, memref<?x32xf32, strided<[384, 1], offset: ?>>, index, index) -> ()
        "scf.yield"() : () -> ()
      }) : (index, index, index) -> ()
      "scf.yield"() : () -> ()
    }) : (index, index, index) -> ()
