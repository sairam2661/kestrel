...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "slice_in_place"}> ({
    ...
    %2 = "hal.interface.binding.subspan"(%0, %1) <{binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 2>}> : (index, index) -> !iree_tensor_ext.dispatch.tensor<readwrite:tensor<?x?xi32>>
    %3 = "iree_tensor_ext.dispatch.tensor.load"(%2, %0, %1, %0, %1) <{operandSegmentSizes = array<i32: 1, 2, 0, 2, 0>, static_offsets = array<i64: 0, 0>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (!iree_tensor_ext.dispatch.tensor<readwrite:tensor<?x?xi32>>, index, index, index, index) -> tensor<?x?xi32>
    "iree_tensor_ext.dispatch.tensor.store"(%3, %2, %0, %1, %0, %1) <{operandSegmentSizes = array<i32: 1, 1, 2, 0, 2, 0>, static_offsets = array<i64: 0, 0>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (tensor<?x?xi32>, !iree_tensor_ext.dispatch.tensor<readwrite:tensor<?x?xi32>>, index, index, index, index) -> ()
    "func.return"() : () -> ()
  }) : () -> ()
}) : () -> ()
