...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "conv_nhwc_dispatch_0"}> ({
    "hal.executable.variant"() <{sym_name = "rocm", target = #executable_target_rocm_hsaco_fb}> ({
      "hal.executable.export"() <{layout = #pipeline_layout, sym_name = "conv_nhwc", sym_visibility = "public"}> ({
      ^bb0(%arg5: !hal.device):
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = () -> (), sym_name = "conv_nhwc"}> ({
          ...
          ^bb0(%arg0: f16, %arg1: f16, %arg2: f32):
            ...
            "linalg.yield"(%13) : (f32) -> ()
          }) {linalg.memoized_indexing_maps = [#map, #map1, #map2], lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_F32_16x16x16_F16>, promote_operands = [0, 1], reduction = [0, 0, 0, 0, 1, 1, 32], subgroup_basis = [[1, 1, 2, 2, 1, 1, 1], [0, 1, 2, 3, 4, 5, 6]], workgroup = [1, 1, 64, 128, 0, 0, 0]}>} : (tensor<2x258x514x768xf16>, tensor<3x3x768x256xf16>, tensor<2x256x512x256xf32>) -> tensor<2x256x512x256xf32>
