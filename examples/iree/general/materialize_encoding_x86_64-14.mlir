...
#encoding = #iree_encoding.encoding<operand_index = 0 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?]>
#encoding1 = #iree_encoding.encoding<operand_index = 1 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?]>
#encoding2 = #iree_encoding.encoding<operand_index = 2 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?]>
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "matmul_lowering_bf16bf16f32_x86_64_avx512bf16"}> ({
    ...
    ^bb0(%arg0: bf16, %arg1: bf16, %arg2: f32):
      %11 = "arith.extf"(%arg0) : (bf16) -> f32
      %12 = "arith.extf"(%arg1) : (bf16) -> f32
      ...
      "linalg.yield"(%14) : (f32) -> ()
    }) : (tensor<?x?xbf16, #encoding>, tensor<?x?xbf16, #encoding1>, tensor<?x?xf32, #encoding2>) -> tensor<?x?xf32, #encoding2>
