...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "gather"}> ({
    ...
    %6 = "hal.interface.binding.subspan"(%2) <{binding = 1 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 1>}> : (index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?xi32>>
    ...
    %10 = "iree_tensor_ext.dispatch.tensor.load"(%6, %2, %2) <{operandSegmentSizes = array<i32: 1, 1, 0, 1, 0>, static_offsets = array<i64: 0>, static_sizes = array<i64: -9223372036854775808>, static_strides = array<i64: 1>}> : (!iree_tensor_ext.dispatch.tensor<readonly:tensor<?xi32>>, index, index) -> tensor<?xi32>
    %11 = "linalg.generic"(%10, %8) <{indexing_maps = [#map, #map1], iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>], operandSegmentSizes = array<i32: 1, 1>}> ({
    ^bb0(%arg0: i32, %arg1: f32):
      %12 = "linalg.index"() <{dim = 1 : i64}> : () -> index
      %13 = "arith.index_cast"(%arg0) : (i32) -> index
      %14 = "tensor.extract"(%9, %13, %12) : (tensor<?x?xf32>, index, index) -> f32
      "linalg.yield"(%14) : (f32) -> ()
    }) : (tensor<?xi32>, tensor<?x?xf32>) -> tensor<?x?xf32>
    "iree_tensor_ext.dispatch.tensor.store"(%11, %7, %3, %4, %3, %4) <{operandSegmentSizes = array<i32: 1, 1, 2, 0, 2, 0>, static_offsets = array<i64: 0, 0>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (tensor<?x?xf32>, !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?xf32>>, index, index, index, index) -> ()
    "func.return"() : () -> ()
  }) : () -> ()
}) : () -> ()
