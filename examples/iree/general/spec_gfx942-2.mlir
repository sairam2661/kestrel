...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "main", sym_visibility = "public"}> ({
    "hal.executable.variant"() <{sym_name = "rocm_hsaco_fb", sym_visibility = "public", target = #executable_target_rocm_hsaco_fb}> ({
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = (tensor<3x10x4096x64xf16>, tensor<3x10x32x64xf16>, tensor<3x10x64x32xf16>) -> tensor<3x10x4096x64xf16>, sym_name = "attention_3x10x4096x64x64x32_f16"}> ({
        ^bb0(%arg0: tensor<3x10x4096x64xf16>, %arg1: tensor<3x10x32x64xf16>, %arg2: tensor<3x10x64x32xf16>):
          %0 = "arith.constant"() <{value = 1.250000e-01 : f16}> : () -> f16
          %1 = "tensor.empty"() : () -> tensor<3x10x4096x64xf16>
          %2 = "iree_linalg_ext.attention"(%arg0, %arg1, %arg2, %0, %1) <{indexing_maps = [#map, #map1, #map2, #map3, #map4]}> ({
          ^bb0(%arg3: f32):
            "iree_linalg_ext.yield"(%arg3) : (f32) -> ()
          }) : (tensor<3x10x4096x64xf16>, tensor<3x10x32x64xf16>, tensor<3x10x64x32xf16>, f16, tensor<3x10x4096x64xf16>) -> tensor<3x10x4096x64xf16>
          "func.return"(%2) : (tensor<3x10x4096x64xf16>) -> ()
        }) : () -> ()
      }) : () -> ()
