...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "i4_i4_i32_matmul"}> ({
    ...
    %5 = "hal.interface.binding.subspan"(%2, %4) <{binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 2>}> : (index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xi4>>
    %6 = "hal.interface.binding.subspan"(%4, %3) <{binding = 1 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 2>}> : (index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xi4>>
    %7 = "hal.interface.binding.subspan"(%2, %3) <{binding = 2 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 2>}> : (index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xi32>>
    %8 = "hal.interface.binding.subspan"(%2, %3) <{binding = 3 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 0, 2>}> : (index, index) -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?xi32>>
    %9 = "iree_tensor_ext.dispatch.tensor.load"(%5, %2, %4, %2, %4) <{operandSegmentSizes = array<i32: 1, 2, 0, 2, 0>, static_offsets = array<i64: 0, 0>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (!iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xi4>>, index, index, index, index) -> tensor<?x?xi4>
    %10 = "iree_tensor_ext.dispatch.tensor.load"(%6, %4, %3, %4, %3) <{operandSegmentSizes = array<i32: 1, 2, 0, 2, 0>, static_offsets = array<i64: 0, 0>, static_sizes = array<i64: -9223372036854775808, -9223372036854775808>, static_strides = array<i64: 1, 1>}> : (!iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?xi4>>, index, index, index, index) -> tensor<?x?xi4>
    ...
    ^bb0(%arg0: i4, %arg1: i4, %arg2: i32):
      %13 = "arith.extsi"(%arg0) : (i4) -> i32
      %14 = "arith.extsi"(%arg1) : (i4) -> i32
      %15 = "arith.muli"(%13, %14) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
      %16 = "arith.addi"(%arg2, %15) <{overflowFlags = #arith.overflow<none>}> : (i32, i32) -> i32
      "linalg.yield"(%16) : (i32) -> ()
    }) : (tensor<?x?xi4>, tensor<?x?xi4>, tensor<?x?xi32>) -> tensor<?x?xi32>
