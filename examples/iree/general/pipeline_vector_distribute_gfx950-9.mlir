...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "attention_mfma_32x32x16", sym_visibility = "private"}> ({
    "hal.executable.variant"() <{sym_name = "rocm", sym_visibility = "public", target = #executable_target_rocm_hsaco_fb}> ({
      "hal.executable.export"() <{layout = #pipeline_layout, ordinal = 0 : index, sym_name = "attention_mfma_32x32x16", sym_visibility = "public"}> ({
      ^bb0(%arg3: !hal.device):
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = () -> (), sym_name = "attention_mfma_32x32x16"}> ({
          ...
          %11 = "iree_linalg_ext.attention"(%6, %7, %8, %0, %10) <{decomposition_config = {pv_attrs = {attention_pv_matmul, lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_F32_32x32x8_F16>, promote_operands = [1], subgroup_basis = [[1, 1, 4, 1, 1, 1], [0, 1, 2, 4, 5]]}>}, qk_attrs = {attention_qk_matmul, lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_F32_32x32x16_F16>, promote_operands = [0, 1], subgroup_basis = [[1, 1, 4, 1, 1, 1], [0, 1, 2, 3, 4]]}>}}, indexing_maps = [#map, #map1, #map2, #map3, #map4]}> ({
          ^bb0(%arg2: f32):
            "iree_linalg_ext.yield"(%arg2) : (f32) -> ()
          }) {lowering_config = #iree_gpu.lowering_config<{promote_operands = [0, 1, 2], reduction = [0, 0, 0, 0, 64, 0], workgroup = [1, 1, 128, 0, 0, 64]}>} : (tensor<24x64x4608x128xf16>, tensor<24x4608x128xf16>, tensor<24x4608x128xf16>, f16, tensor<24x64x4608x128xf16>) -> tensor<24x64x4608x128xf16>
