...
"builtin.module"() ({
  "hal.executable"() <{sym_name = "matvec_fp16_parallel_subgroup", sym_visibility = "private"}> ({
    "hal.executable.variant"() <{sym_name = "rocm_hsaco_fb", sym_visibility = "public", target = #executable_target_rocm_hsaco_fb}> ({
      "hal.executable.export"() <{layout = #pipeline_layout, ordinal = 0 : index, sym_name = "matvec_fp16_parallel_subgroup", sym_visibility = "public"}> ({
      ^bb0(%arg5: !hal.device):
      ...
      "builtin.module"() ({
        "func.func"() <{function_type = () -> (), sym_name = "matvec_fp16_parallel_subgroup"}> ({
          ...
          ^bb0(%arg0: f16, %arg1: f16, %arg2: f16):
            ...
            "linalg.yield"(%11) : (f16) -> ()
          }) {lowering_config = #iree_gpu.lowering_config<{lane_basis = [[1, 1, 64], [0, 1, 2]], reduction = [0, 0, 512], subgroup_basis = [[1, 4, 1], [0, 1, 2]], thread = [0, 0, 8], workgroup = [1, 4, 0]}>} : (tensor<1x4096xf16>, tensor<32000x4096xf16>, tensor<1x32000xf16>) -> tensor<1x32000xf16>
