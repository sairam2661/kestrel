...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "dequant_matmul"}> ({
    ...
    %10 = "tensor.empty"() : () -> tensor<4x256x128x1xf32>
    %11 = "linalg.generic"(%8, %10) <{indexing_maps = [#map, #map], iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>], operandSegmentSizes = array<i32: 1, 1>}> ({
    ^bb0(%arg5: i8, %arg6: f32):
      %16 = "arith.uitofp"(%arg5) : (i8) -> f32
      "linalg.yield"(%16) : (f32) -> ()
    }) : (tensor<4x256x128x1xi8>, tensor<4x256x128x1xf32>) -> tensor<4x256x128x1xf32>
    %12 = "linalg.fill"(%3, %9) <{operandSegmentSizes = array<i32: 1, 1>}> ({
    ^bb0(%arg3: f32, %arg4: f32):
      "linalg.yield"(%arg3) : (f32) -> ()
    }) : (f32, tensor<1x4x1x128xf32>) -> tensor<1x4x1x128xf32>
    %13 = "linalg.mmt4d"(%7, %11, %12) <{operandSegmentSizes = array<i32: 2, 1>}> ({
    ^bb0(%arg0: f32, %arg1: f32, %arg2: f32):
      ...
      "linalg.yield"(%15) : (f32) -> ()
    }) {linalg.memoized_indexing_maps = [#map1, #map2, #map3]} : (tensor<1x256x1x1xf32>, tensor<4x256x128x1xf32>, tensor<1x4x1x128xf32>) -> tensor<1x4x1x128xf32>
    "iree_tensor_ext.dispatch.tensor.store"(%13, %6) <{operandSegmentSizes = array<i32: 1, 1, 0, 0, 0, 0>, static_offsets = array<i64: 0, 0, 0, 0>, static_sizes = array<i64: 1, 4, 1, 128>, static_strides = array<i64: 1, 1, 1, 1>}> : (tensor<1x4x1x128xf32>, !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x4x1x128xf32>>) -> ()
    "func.return"() : () -> ()
