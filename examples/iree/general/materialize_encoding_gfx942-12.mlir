...
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "batch_matmul_lowering_MFMA_F32_16x16x4_F32"}> ({
    ...
    %5 = "hal.interface.binding.subspan"(%0, %1, %2, %4) <{alignment = 64 : index, binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xf32, #encoding>>
    %6 = "hal.interface.binding.subspan"(%0, %1, %4, %3) <{alignment = 64 : index, binding = 1 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xf32, #encoding1>>
    ...
    ^bb0(%arg0: f32, %arg1: f32, %arg2: f32):
      ...
      "linalg.yield"(%13) : (f32) -> ()
    }) : (tensor<?x?x?xf32, #encoding>, tensor<?x?x?xf32, #encoding1>, tensor<?x?x?xf32, #encoding2>) -> tensor<?x?x?xf32, #encoding2>
