...
#encoding = #iree_encoding.encoding<operand_index = 0 : i64, op_type =  matmul, element_types = [f8E4M3FNUZ, f8E4M3FNUZ, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?, ?]>
#encoding1 = #iree_encoding.encoding<operand_index = 1 : i64, op_type =  matmul, element_types = [f8E4M3FNUZ, f8E4M3FNUZ, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?, ?]>
#encoding2 = #iree_encoding.encoding<operand_index = 2 : i64, op_type =  matmul, element_types = [f8E4M3FNUZ, f8E4M3FNUZ, f32], user_indexing_maps = [#map, #map1, #map2], iteration_sizes = [?, ?, ?, ?]>
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "batch_matmul_lowering_MFMA_F32_16x16x32_F8E4M3FNUZ"}> ({
    ...
    %5 = "hal.interface.binding.subspan"(%0, %1, %2, %4) <{alignment = 64 : index, binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xf8E4M3FNUZ, #encoding>>
    %6 = "hal.interface.binding.subspan"(%0, %1, %4, %3) <{alignment = 64 : index, binding = 1 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xf8E4M3FNUZ, #encoding1>>
    ...
    ^bb0(%arg0: f8E4M3FNUZ, %arg1: f8E4M3FNUZ, %arg2: f32):
      %12 = "arith.extf"(%arg0) : (f8E4M3FNUZ) -> f32
      %13 = "arith.extf"(%arg1) : (f8E4M3FNUZ) -> f32
      ...
      "linalg.yield"(%15) : (f32) -> ()
    }) : (tensor<?x?x?xf8E4M3FNUZ, #encoding>, tensor<?x?x?xf8E4M3FNUZ, #encoding1>, tensor<?x?x?xf32, #encoding2>) -> tensor<?x?x?xf32, #encoding2>
