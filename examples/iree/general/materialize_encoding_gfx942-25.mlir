...
#encoding = #iree_encoding.encoding<operand_index = 0 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2]>
#encoding1 = #iree_encoding.encoding<operand_index = 1 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2]>
#encoding2 = #iree_encoding.encoding<operand_index = 2 : i64, op_type =  matmul, element_types = [bf16, bf16, f32], user_indexing_maps = [#map, #map1, #map2]>
"builtin.module"() ({
  "func.func"() <{function_type = () -> (), sym_name = "batch_matmul_lowering_MFMA_F32_16x16x16_BF16"}> ({
    ...
    %5 = "hal.interface.binding.subspan"(%0, %1, %2, %4) <{alignment = 64 : index, binding = 0 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xbf16, #encoding>>
    %6 = "hal.interface.binding.subspan"(%0, %1, %4, %3) <{alignment = 64 : index, binding = 1 : index, layout = #pipeline_layout, operandSegmentSizes = array<i32: 1, 3>}> : (index, index, index, index) -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x?xbf16, #encoding1>>
    ...
    ^bb0(%arg0: bf16, %arg1: bf16, %arg2: f32):
      %12 = "arith.extf"(%arg0) : (bf16) -> f32
      %13 = "arith.extf"(%arg1) : (bf16) -> f32
      ...
      "linalg.yield"(%15) : (f32) -> ()
    }) : (tensor<?x?x?xbf16, #encoding>, tensor<?x?x?xbf16, #encoding1>, tensor<?x?x?xf32, #encoding2>) -> tensor<?x?x?xf32, #encoding2>
