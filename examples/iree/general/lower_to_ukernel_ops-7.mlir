...
"builtin.module"() ({
  "func.func"() <{function_type = (tensor<?x?x16x2xi8>, tensor<?x?x16x2xi8>, tensor<?x?x16x16xi32>) -> tensor<?x?x16x16xi32>, sym_name = "mmt4d_i8i8i32"}> ({
  ^bb0(%arg0: tensor<?x?x16x2xi8>, %arg1: tensor<?x?x16x2xi8>, %arg2: tensor<?x?x16x16xi32>):
    ...
    ^bb0(%arg3: i8, %arg4: i8, %arg5: i32):
      ...
      "linalg.yield"(%4) : (i32) -> ()
    }) {linalg.memoized_indexing_maps = [#map, #map1, #map2]} : (tensor<?x?x16x2xi8>, tensor<?x?x16x2xi8>, tensor<?x?x16x16xi32>) -> tensor<?x?x16x16xi32>
