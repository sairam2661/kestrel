"builtin.module"() ({
  "firrtl.circuit"() <{annotations = [], name = "LocalPath"}> ({
    "firrtl.module"() <{annotations = [], convention = #firrtl<convention internal>, domainInfo = [], layers = [], portAnnotations = [], portDirections = array<i1>, portLocations = [], portNames = [], portSymbols = [], portTypes = [], sym_name = "LocalPath"}> ({
      %1 = "firrtl.unresolved_path"() <{target = "OMInstanceTarget:~LocalPath|Child"}> : () -> !firrtl.path
      %2 = "firrtl.unresolved_path"() <{target = "OMReferenceTarget:~LocalPath|Child>wire"}> : () -> !firrtl.path
